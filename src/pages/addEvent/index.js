...
import { Create_Update_Doc, imageUploading } from "@/firebase/firebaseutils";
...
const EventForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    const startDate = Timestamp.fromDate(new Date(formData.date));
    const endDate = Timestamp.fromDate(new Date(formData.closeDate));
    ...
    const formLatLong = new GeoPoint(formLat, formLong);
    const userIdString = localStorage.getItem("userId");
    const userRef = doc(db, "users", userIdString);

    // Add the timestamps to formData
    let updatedFormData = {
      ...formData,
      city: formCity,
      location: formAdress,
